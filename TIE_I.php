...
.center{
    ...
    <body>
         <div style="background-color: black;width: 700px;">
          <img src='I_TIE.jpg' height="200" width="200" class="center"><br><br>
        <h1 aligh="center"; style="color:white;margin-left: 220px;">TIE Interceptor<br><br>
         </div>
        <?php
         include 'TieFighter.php';   
         include 'TieFighter_I.php' ;
        ...
        // connection successfull
          $result = $connDB->query("SELECT m.Model, m.Class, m.Cost, m.Manufacturer,
                                        ...
                                        ON m.Model = ar.Model
                                        where m.Model = 'Tie Fighter I' ");
 
        if($result->num_rows > 0){
         while ($row_val = $result->fetch_assoc()){
            $tieIObj= new TieFighter_I( $row_val["Model"], $row_val["Class"], $row_val["Cost"], $row_val["Manufacturer"], $row_val["Length"], $row_val["Width"], $row_val["Height"], $row_val["Mass"], $row_val["Crew"], $row_val["MaxSpeed"], $row_val["MaxClimb"], $row_val["Armament"], $row_val["Propulsion"], $row_val["Hull"],' ','',' ');

          
          echo "<h2>" .$tieIObj->displayFighterInfo(). "</h2>";
        }
        }else{
        ...
        <button type="button" class="button4" onclick="location= 'IInventory.php'"> I Inventory</button>
        </form> 
        </div>
      </body>
    </html>  
